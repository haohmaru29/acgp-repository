...
CambiaValor : function(Valor){
	...
	//alert ("CambiaValor=" + colObj.id);	
	this.updateRecordField(this.activeCell ,Valor );
	this.refreshRow(row,this.activeRecord);
	this.dirty(this.activeCell);
	this.activeValue=Valor;
				
	Sigma.$invoke(this.activeColumn,'afterEdit',[Valor,oldValue,this.activeRecord,this.activeColumn,this]);
	Sigma.$invoke(this,'afterEdit',[Valor,oldValue,this.activeRecord,this.activeColumn,this]);
...
CalculoTotalGrilla : function(event) {
	var grid=this;
	...
	if (IntTipoProd>=1 && IntTipoProd<=5)
	{
		for (var k=0,len=rows.length; k<len; k++) 
		{
			...
			{ 	//validar datos		
				switch (IntTipoProd){
					...
					case 3:
					case 4:
						total=total + parseFloat(valJS.quitaMask(recordN[3] + ''));
						break;			
				}
			}	
		}
...
validarDatosGrilla : function(event) {
	...
	for (var k=0,len=rows.length; k<len; k++) 
	{
		...
		if (recordN) { //validar datos			
			...
			if (flagsigue) {
				//alert(flagsigue);
				flagsigue=grid.validarDatosEntrada(event,false,k);
				//alert("flagsigue=" + flagsigue );	
				if (flagsigue==false) {
					total=-1;
					break;
				}
...
EliminarRemesaCosta : function (sTipoProd,snumremesa,sabogado) 
{
			try {
				datastring= "TIPOPROD=" + sTipoProd + "&NUMREMESA=" + snumremesa + "&ABO=" + sabogado ;	
				jQuery.ajax({ 								
						timeout: 60000,
						dataType: 'JSON',					
			            url: "../jsp/AJAX_EliminarRemesaCosta.jsp",
			            data: datastring,	
			            async:false,		          
...
GrabaCostaDetHost : function (sTipoProd,snumcosta,sabogado,sRut,soperacion,svalorcosta,stipocargo,ssubtipocargo,sNumJuicio,sTipoJuicio) {
			try {
				sRut = sRut.toUpperCase();
				sRut = sRut.replace(/\.|-/ig,'');	
				svalorcosta = svalorcosta.replace(/\.|-/ig,'');				
				datastring= "TIPOPROD=" + sTipoProd + "&NUMCOSTA=" + snumcosta + "&ABO=" + sabogado + "&RUT=" + sRut + "&OPE=" + soperacion + "&MONTO=" + svalorcosta + "&TIPCARGO=" + stipocargo + "&SUBTIPCARGO=" + ssubtipocargo + "&NUMJUICIO=" + sNumJuicio + "&TIPOJUICIO=" + sTipoJuicio;
				//alert(datastring);	
				jQuery.ajax({ 								
						timeout: 60000,
						dataType: 'JSON',					
			            url: "../jsp/AJAX_GrabaDetalleCosta.jsp",
			            data: datastring,	
			            async:false,		          
			            beforeSend:function(xhr){}, 
			            success:function(data){	   			            	
			              try {    
			              		var obj = jQuery.parseJSON(data);		
			              		document.getElementById("RESPBD").value=obj.total;	
			              		//alert("detalle->obj.total=" + obj.total);		              	
			              		if (obj.total==-1) 
			            	    {			            	    	
			            	    	validResult=[].concat("Error al grabar detalle de la Costa.");   
...
GrabaCostaEncHost : function (susuario,sprestador,sabogado,stipodoc,snumdoc,smonto,smoneda,sTipoProd,sGrupo) {
			try {
				sprestador = sprestador.toUpperCase();
				sprestador = sprestador.replace(/\.|-/ig,'');	
				smonto = smonto.replace(/\.|-/ig,'');	
				datastring= "USUARIO=" + susuario + "&PRESTADOR=" + sprestador + "&ABO=" + sabogado + "&TIPODOC=" + stipodoc + "&NUMDOC=" + snumdoc + "&MONTO=" + smonto + "&MONEDA=" + smoneda + "&TIPOPROD=" + sTipoProd + "&GRUPO=" + sGrupo;	
				//alert(datastring);	
				jQuery.ajax({ 								
						timeout: 60000,
						dataType: 'JSON',					
			            url: "../jsp/AJAX_GrabaEncabezadoCosta.jsp",
			            data: datastring,	
			            async:false,		          
			            beforeSend:function(xhr){}, 
			            success:function(data){	   			            	
			              try {    
			              		var obj = jQuery.parseJSON(data);		
			              		//alert("encabezado->obj.total=" + obj.total);
			              		document.getElementById("RESPBD").value=obj.total;			              	
			              		if (obj.total==-1) {			            	    	
			            	    	validResult=[].concat("Error al grabar encabezado de la Costa.");   
			            	    	document.getElementById("TxtMensajeError").value=validResult.join('\n');   
			            	    	return false;
			            	    } 
			            	    else {			 
...
GrabarCosta : function(event,sGrupo) 
{
	//grabar encabezado
	var grid=this;
	grid.GrabaCostaEncHost(document.getElementById("TxtUser").value,document.getElementById("TxtRutPrestador").value,document.getElementById("HDDCODABO").value,document.getElementById("HDDTIPODOCM").value,document.getElementById("TxtNumDocum").value,document.getElementById("TxtMonto").value,document.getElementById("HDDMONEDA").value,document.getElementById("TIPOPROD").value,sGrupo);
	var NumCosta=parseFloat(document.getElementById("RESPBD").value);
	if (NumCosta>0)
	{
		//grabar detalle
		...
		for (var k=0,len=rows.length; k<len; k++) 
		{
			...
			if (recordN)		
			{ 
				if (recordN[1]!="") // grabar detalle
				{
					grid.GrabaCostaDetHost(document.getElementById("TIPOPROD").value,NumCosta,document.getElementById("HDDCODABO").value,recordN[1],recordN[2],recordN[3],recordN[4],recordN[5],recordN[6],recordN[7]);
					if (document.getElementById("RESPBD").value=="-1" || document.getElementById("RESPBD").value=="-2")
					{
						//si hubo error en el detalle eliminar remesa
						grid.EliminarRemesaCosta(document.getElementById("TIPOPROD").value,NumCosta,document.getElementById("HDDCODABO").value);
						break;
					}	
				}
...
GrabaRemesaDetHost : function (sTipoProd,snumremesa,sabogado,sRut,soperacion,scapital,sinteres,scostas,shonorario,sboleta,stotal) 
{
			try {
				sRut = sRut.toUpperCase();
				sRut = sRut.replace(/\.|-/ig,'');	
				/*
				alert("sRut=" + sRut);
				alert("snumremesa=" + snumremesa);
				alert("scapital=" + scapital);
				alert("sinteres=" + sinteres);
				alert("scostas=" + scostas);
				alert("shonorario=" + shonorario);
				alert("stotal=" + stotal);
				*/
				scapital = scapital.replace(/\.|-/ig,'');		
				sinteres = sinteres.replace(/\.|-/ig,'');	
				scostas = scostas.replace(/\.|-/ig,'');		
				shonorario = shonorario.replace(/\.|-/ig,'');	
				stotal = stotal.replace(/\.|-/ig,'');	
				datastring= "TIPOPROD=" + sTipoProd + "&NUMREMESA=" + snumremesa + "&ABO=" + sabogado + "&RUT=" + sRut + "&OPE=" + soperacion + "&CAPITAL=" + scapital + "&INTERES=" + sinteres + "&COSTA=" + scostas + "&HONO=" + shonorario + "&BOLETA=" + sboleta + "&TOTAL=" + stotal;	
				//alert(datastring);	
				jQuery.ajax({ 								
						timeout: 60000,
						dataType: 'JSON',					
			            url: "../jsp/AJAX_GrabaDetalleRemesa.jsp",
			            data: datastring,	
			            async:false,		          
			            beforeSend:function(xhr){}, 
			            success:function(data){	   			            	
			              try {    
			              		...
			              		if (obj.total==-1) 
			            	    {			            	    	
			            	    	validResult=[].concat("Error al grabar detalle de la Remesa.");   
			            	    	document.getElementById("TxtMensajeError").value=validResult.join('\n');   
			            	    	return false;
			            	    } 
			            	    else
...
GrabaRemesaEncHost : function (susuario,sfecharemesa,snuminterno,sabogado,smonto,smoneda,sTipoProd,sGrupo) {
			try {
				smonto = smonto.replace(/\.|-/ig,'');
				...
				jQuery.ajax({ 								
						timeout: 60000,
						dataType: 'JSON',					
			            url: "../jsp/AJAX_GrabaEncabezadoRemesa.jsp",
			            data: datastring,	
			            async:false,		          
			            beforeSend:function(xhr){}, 
			            success:function(data){	   			            	
			              try {    
			              		...
			              		if (obj.total==-1) 
			            	    {			            	    	
			            	    	validResult=[].concat("Error al grabar encabezado de Remesa.");   
			            	    	document.getElementById("TxtMensajeError").value=validResult.join('\n');   
			            	    	return false;
			            	    } 
			            	    else
...
GrabarRemesa : function(event,sGrupo) 
{
	//grabar encabezado
	var grid=this;
	grid.GrabaRemesaEncHost(document.getElementById("TxtUser").value,document.getElementById("TxtFechaRemesa").value,document.getElementById("TxtNumInterno").value,document.getElementById("HDDCODABO").value,document.getElementById("TxtMonto").value,document.getElementById("HDDMONEDA").value,document.getElementById("TIPOPROD").value,sGrupo);
	var NumRemesa=parseFloat(document.getElementById("RESPBD").value);
	//alert("NumRemesa=" + document.getElementById("RESPBD").value);
	//alert("NumRemesa=" + NumRemesa);
	if (NumRemesa>0)
	{
		//grabar detalle
		document.getElementById("RESPBD").value="";
			...
			{ 
				if (recordN[1]!="") // grabar detalle
				{
					...
					if (TipoProd=="1" || TipoProd=="2") {
						grid.GrabaRemesaDetHost(document.getElementById("TIPOPROD").value,NumRemesa,document.getElementById("HDDCODABO").value,recordN[1]+ '',recordN[2]+'',recordN[3]+'',recordN[4]+'',recordN[5]+'',recordN[6]+'',recordN[7]+'',recordN[8]+'');
						
					}else //preabogado
						grid.GrabaRemesaDetHost(document.getElementById("TIPOPROD").value,NumRemesa,document.getElementById("HDDCODABO").value,recordN[1]+ '',recordN[2]+'',recordN[3]+'',recordN[4]+'','0','0','0',recordN[5]+'');
					
					if (document.getElementById("RESPBD").value=="-1" || document.getElementById("RESPBD").value=="-2")
					{
						//si hubo error en el detalle eliminar remesa
						grid.EliminarRemesaCosta(document.getElementById("TIPOPROD").value,NumRemesa,document.getElementById("HDDCODABO").value);
						break;
					}	
				}
...
SeleccionaCampoGrilla : function (NumFila,NumReg)
	{
		var grid=this;
		Sigma.U.stopEvent(event);
  		grid.unselectAllRow(); //deselecciona la fila le quita el color de seleccion
	  	//grid.activeMe();		
	  	grid.endEdit(1) //1 =no borra mensajes
	  	var rows=grid.getAllRows();
		var Cell=rows[NumFila].cells[NumReg];	
		grid.selectRow(rows[NumFila],true);
		grid.activeMe();
		var TipoProd=document.getElementById("TIPOPROD");
		if (TipoProd.value=="1" || TipoProd.value=="3" || TipoProd.value=="5") //seleccionar numero de operacion
		{
			if (NumReg==2)//es operacion
			{	
...
validarDatosEntrada : function(event,filaactiva,numfilavalidar) {
	var grid=this;
	var CampoEnter=document.getElementById("CampoEnter");	 	 	
	var flagsigue=true;
	var valorfilaactiva=0;
	var objMen=document.getElementById("TxtMensajeError");	 
	if (filaactiva)
	{
		var rows=grid.getRows();
		var recordN=grid.getRecordByRow(this.activeRow);
		var RegistroActivo=this.activeRow;
		valorfilaactiva=this.activeRow.rowIndex;
	}
	else
	{
		//selecciono la ultima fila
		var rows=grid.getRows();
		var recordN=grid.getRecordByRow(rows[numfilavalidar]);
		var RegistroActivo=rows[numfilavalidar];		
		valorfilaactiva=numfilavalidar;
	}

	//validar que rut sea correcto		
	objMen.value="";	   
	if (valJS.isRutValido(recordN[1])==false)
	{
		flagsigue=false;
		objMen.value="Rut no v�lido, favor digite nuevamente.";
		grid.SeleccionaCampoGrilla(valorfilaactiva,1);
	}
	else
	{
		//validar seleccion TARJETA (1,3,5) O CHEQUE INGRESADO (2,4)
		var TipoProd=document.getElementById("TIPOPROD");
		if (TipoProd.value=="1" || TipoProd.value=="3" || TipoProd.value=="5") // valida seleccion tarjeta remesa y costa
		{
			if (recordN[2]=="")
			{
				flagsigue=false;
				objMen.value="Debe seleccionar N�operaci�n.";
				grid.SeleccionaCampoGrilla(valorfilaactiva,2);
			}						  											  				
		}	
		if ((flagsigue==true) && (TipoProd.value=="2" || TipoProd.value=="4")) // valida cheque remesa y costa
		{
			if (recordN[2]=="")
			{
				flagsigue=false;
			...
			else
			{
				...
				mygrid.ValidaNumCheque(recordN[2]);
				var respcosta=parseInt(document.getElementById("RESPBD").value);
				if (respcosta<0)
				{
					if (respcosta==-1)
					{
					...
					else
					{
						...
						{
							case -2:
								objMen.value="N� Unico debe ser num�rico.";
								break;
							case -3:
		...
		//VALIDACION DE HONORARIO SI A INGRESADO BOLETA
		if (flagsigue==true && (TipoProd.value=="1" || TipoProd.value=="2")) // remesa (tarjeta,cheque)
		{
		...
		if (flagsigue==true && (TipoProd.value=="1" || TipoProd.value=="2" || TipoProd.value=="5")) // remesa (tarjeta,cheque,preabogado), valida los montos totales
		{
			if (document.getElementById("NUMREGTOTAL").value!="0" && recordN[document.getElementById("NUMREGTOTAL").value]==0)
			{
				flagsigue=false;
			    objMen.value="Total del registro debe ser mayor a cero.";
				grid.SeleccionaCampoGrilla(valorfilaactiva,CampoEnter.value);	
			}
		}
		//alert("flagsigue=" + flagsigue);
		//alert("TipoProd.value=" + TipoProd.value);
		if (flagsigue==true && (TipoProd.value=="3" || TipoProd.value=="4")) //si es costa (tarjeta, cheque) valida valor costa >0
		{
			//alert("recordN[3]=" + recordN[3]);
			if (recordN[3]==0)
			{
				//flagsigue=false;
				//objMen.value="Valor Costa debe ser mayor a cero.";
				//grid.SeleccionaCampoGrilla(valorfilaactiva,3);	
			}
		}			
		
		//validar tipo de cargo para la costa
		if (flagsigue==true && (TipoProd.value=="3" || TipoProd.value=="4")) //si es costa validar tipo de cargo
		{
			if (recordN[4]=="")
			{
				flagsigue=false;
				objMen.value="Debe ingresar Tipo de Cargo.";
				grid.SeleccionaCampoGrilla(valorfilaactiva,4);	
			}
			else
			{
				//validar tipo de cargo al servidor
				//record[7] tipo de juicio
				grid.ValidaTipoCargo(TipoProd.value,recordN[4],recordN[7]);
				...
				else
				{
					...
					{
						if (recordN[5]!=document.getElementById("RESPBD").value)
						{   //cambiar valor
							var colObj= this.getColumn(RegistroActivo.cells[5]);
							this.updateRecordField(RegistroActivo.cells[5] ,document.getElementById("RESPBD").value );
							this.refreshRow(RegistroActivo,recordN);
							this.dirty(RegistroActivo.cells[5]);
							this.activeValue=document.getElementById("RESPBD").value;
						}
						document.getElementById("RESPBD").value="";
					}
				}			
...
ValidarNumeroCheque : function(event,sIndice,sNumCheque) {
	var grid=this;
	var rows=grid.getRows();
	...
	for (var k=0,len=rows.length; k<len; k++) 
	{
		...
		var recordN=grid.getRecordByRow(row);
		if (recordN)		
		{ 	//validar numero cheque	
			if (recordN[0]!=sIndice)
			{			
				if (recordN[2]==sNumCheque)
					return false;
			}			
		}	
	}	
...
ColocarDataSelectOperacion : function (Posicion) {
			...
			while (strrespbd.indexOf(",") >=0)
			{
				i=strrespbd.indexOf(",");
				if (j==0)
					primerelemento=strrespbd.substring(0,i);
				salida=salida + "'" + strrespbd.substring(0,i) + "':'" + strrespbd.substring(0,i) + "',";
				strrespbd=strrespbd.substring(i+1);
				j=j+1;										
			}
			...
			{
				salida="{" + salida + "}";
				vardefault=primerelemento;
			}										
			opciones=eval('(' + salida + ')');		
			if (Posicion=="R")						
				var newCell= mygrid._nextEditableCell(mygrid.activeCell);
			else
				if (Posicion=="L")		
					var newCell= mygrid._prveEditableCell(mygrid.activeCell);
				else
					var newCell= mygrid.activeCell;
					
			var colObj2= mygrid.getColumn(newCell);
			colObj2.editor.valueDom=Sigma.U.createSelect( opciones ,vardefault,{ className:'gt-editor-text' });
			colObj2.editor.dom.appendChild(colObj2.editor.valueDom);		
			vardefault='';		
			opciones= {'': ''};		
},
		
sleep :	function sleep(millisegundos) {
var inicio = new Date().getTime();
while ((new Date().getTime() - inicio) < millisegundos);
},

	 _onKeydown : function(event) {
	 	 		var NumFila=document.getElementById("NUMFILA");
	 	 		var NumReg=document.getElementById("NUMREG");
	 	 		NumFila.value="0";	
	 	 		NumReg.value="1";									   
	     		var grid=this;
				var oldCell=grid.activeCell;
				var newCell=null;
				var kcode=event.keyCode;	
				var et =Sigma.U.getEventTarget(event);		
					//inicia edicion de celda
					function editCell(_cell){	
						if (_cell){
							Sigma.U.stopEvent(event);
							if (grid.editing) {
							   flag=grid.endEdit(0);		
							   if (flag!==true) {//avanza al siguiente campo
								    _cell = oldCell;
								    newCell=oldCell;
								 }
						  }
					...
					//alert("fffkcode=" + kcode);
					switch (kcode){						
						case Sigma.Const.Key.TAB:				
							//alert("presiono por select");		
							break;
						case Sigma.Const.Key.F1:	
						case Sigma.Const.Key.NUMLOCK: //AYUDA F1							
							if (document.getElementById("TipoGrilla").value=="EDITABLE")
							{
								//CambiaValor
								var thisEdit = this;
								if (this.activeColumn.id=="TipCargo") // es tipo de cargo
								{
									var recordN=grid.getRecordByRow(this.activeRow);
									//alert(recordN[7]);
									var datastring3='codProducto=' + recordN[2];
									jQuery.ajax({
										   url: "../jsp/AJAX_Tipo_Juicio_Producto.jsp",
										   data: datastring3,
										   timeout: 60000,
										   dataType: 'JSON',
										   beforeSend:function(){},
										   ...
										   success: function(data){
										    	...
										    	var testObj= recordN[7];
										    	//alert(testObj);
												var cont = 0;
										    	var result=window.showModalDialog("Seleccion.jsp?txtJuicio=" + testObj , 0, "dialogWidth:660px;dialogHeight:540px;resizable:no;toolbar:no;menubar:no;scrollbars:yes;help:no");
										    	//Sigma.U.stopEvent(event);
											 	var valorfilaactiva=thisEdit.activeRow.rowIndex;
												if (result.split('|')[0]!="0") {
								 					grid.CambiaValor(result.split('|')[0]);
													newCell= grid._nextEditableCell(oldCell);	
													grid.editCell(newCell);
													grid.CambiaValor(result.split('|')[1]);
												...
												if (kcode==Sigma.Const.Key.NUMLOCK && result.split('|')[2]=="INACTIVO")	
								 				{	
								 					try {
									 					WshShell.SendKeys("{NUMLOCK}");
								 					} catch(e) {}
													document.getElementById("Seleccion").value="INACTIVA";
													grid.SeleccionaCampoGrilla(valorfilaactiva,4);
								 				}
								 				else {
								 					document.getElementById("Seleccion").value="ACTIVA";
													grid.SeleccionaCampoGrilla(valorfilaactiva,4);
								 				}
									      }
									 });
						...
						case Sigma.Const.Key.F3: //eliminar
								if (document.getElementById("TipoGrilla").value=="EDITABLE")
								{
									var filaactiva=this.activeRow;
									var recordN=grid.getRecordByRow(this.activeRow);
									var rows=grid.getRows();
									
									if (rows.length!=1) //solo eliminar si hay mas de una fila
									{
									  //if (recordN[1]!='')
									  //{
											if (confirm("Desea Eliminar fila seleccionada?"))
											{
												//var newActiveCell = Sigma.U.getParentByTagName("td",null, event);	
												//alert("newActiveCell=" + newActiveCell);
												var ARREGLO_DATA=[];
										  		var rows=grid.getRows();
										  		var NumFilaActiva=filaactiva.rowIndex;
										  		var i=0;
										  		//alert("NumFilaActiva=" + NumFilaActiva)
												for (var k=0,len=rows.length; k<len; k++) 
												{
													if (k!=NumFilaActiva)
													{
														...
														if (recordN)			
														{
															//alert(recordN[1] + ";" + recordN[2] + ";" + recordN[3]);
															recordN[0]=i+1;
															ARREGLO_DATA[i]=recordN;		
															i=i+1;												
														}
													}		
												}		
												...
												newCell= grid._nextEditableCell(oldCell);	
												if (newCell)		
													editCellElim(newCell);
												//recorro hasta el final	
												while (newCell) 
												{
												    newCell= grid._nextEditableCell(newCell);
												    if (newCell)	
												  		editCellElim(newCell);	
												}
												grid.deleteRow(filaactiva);
											    //NumFila.value="0"; //primera fila	
												NumFila.value=eval(i-1); //�ltima fila
												NumReg.value="1";	
						...
						if (document.getElementById("TipoGrilla").value=="EDITABLE")
						{
							...
							Sigma.U.stopEvent(event);							
							newCell= grid._nextEditableCell(oldCell);
							var or=oldCell.parentNode,nr=(!newCell)?null:newCell.parentNode;
							if (nr==null) {
						  	} 
						  	else 
						  	{
						  	  //var initCell=oldCell;							
							  if (!newCell) return;		
							  //var ColumActivaAnte=this.activeColumn.fieldIndex;							
							  editCell(newCell);								 
						     ...
						     /* if (ColumActivaAnte==1 && document.getElementById("TxtMensajeError").value!="Rut no v�lido, favor digite nuevamente.")
						      {
						      	var recordN=grid.getRecordByRow(this.activeRow);
						        ...
						        var SubProd=IndiceSub[IndiceSub.selectedIndex].value;	
						      	// validar que RUT sea v�lido en la base de datos
						      	grid.NroCuentasCliente(recordN[1],Abogado,SubProd);
							  	
							  	if (document.getElementById("TxtMensajeError").value!="")
							  	{							  		
							  		NumFila.value=this.activeRow.rowIndex; //numero fila activa
								    NumReg.value="1";	//rut
								  	grid.refresh();		
							  	}
							  }
							  */
							  ...
							  //alert("this.activeColumn.fieldIndex=" + this.activeColumn.fieldIndex);								 						  	 	
							  while (!grid.editing && newCell) //avanza para los que no son editables
							  {
							      newCell= grid._nextEditableCell(newCell);								      
							      editCell(newCell);							     				
							  }
							  ...
							  {
							  			var rows=grid.getRows();	
							  			flagsigue=grid.validarDatosEntrada(event,false,rows.length-1);
							  			//flagsigue=grid.validarDatosGrilla(event);							  			
							  			if(flagsigue)
								  		{	  
								  			//agregar nueva fila
								  			var TopeFilas=document.getElementById("CantidadFilas");
											if (rows.length+1 >TopeFilas.value)
											{
												//NO ES POSIBLE COLOCAR MAS lineas en la grilla
												document.getElementById("TxtMensajeError").value="No es posible agregar m�s l�neas, el total se encuentra en su tope m�ximo.";
											...
											{	
												...
												for (var k=0,len=rows.length; k<len; k++) 
												{
													...
													var recordN=grid.getRecordByRow(row);
													if (recordN)			
														__TEST_DATA2__[k]=recordN;
												} 	
												var TipoProd=document.getElementById("TIPOPROD");			
												if (TipoProd.value=="1" || TipoProd.value=="2") // remesa 
													__TEST_DATA2__[k]=[k+1,"","",0,0,0,0,0,0];
												else					
												{
													if (TipoProd.value=="3" || TipoProd.value=="4") // costa
														__TEST_DATA2__[k]=[k+1,"","",0,"","","",""];
													else // remesa pre abogado
														__TEST_DATA2__[k]=[k+1,"","",0,0,0];
											  			
											  	}		
											  	/*alert(__TEST_DATA2__.length);
											  	for(var i=0 ;i<__TEST_DATA2__.length;i++)
											  	{
											  		alert(__TEST_DATA2__[i]);
											  	}*/		
											  	NumFila.value=k;	
											  	NumReg.value="1";	
												grid.refresh(	__TEST_DATA2__);											
											    grid.refreshGrid(grid);		
											 }   							     
								   		}
								   } 
						...
						if (document.getElementById("TipoGrilla").value=="EDITABLE")
						{
							...
							if ((colObj.id=='ValCosta' || colObj.id=='Capital') && (TipoProd.value=="1" || TipoProd.value=="3" || TipoProd.value=="5"))//solo para tarjeta
							{
								//alert("LEFT->colObj.id=" + colObj.id);
							 	//VOLVER A CARGAR DATOS de operacion
								var recordN=grid.getRecordByRow(this.activeRow);
								if (TipoProd.value=="1" || TipoProd.value=="3") {
									...
									mygrid.NroCuentasClientePreAbo(recordN[1],document.getElementById("HDDGRUPO").value);
							    ...
							    newCell= grid._prveEditableCell(oldCell);		
								var or=oldCell.parentNode,nr=(!newCell)?null:newCell.parentNode;
								if (nr==null) {								
								  ...
								  {							
								      ...
								      newCell= grid._prveEditableCell(newCell);	
								      if (newCell) 
								      	editCell(newCell);
								      else
								      {	
								      	newCell= grid._nextEditableCell(initCell);		
								      	editCell(newCell);
								      }	
								  }
								}
						...
						if (document.getElementById("TipoGrilla").value=="EDITABLE")
						{
						    if (Sigma.U.getTagName(et)!=='INPUT')  return;
						    Sigma.U.stopEvent(event); 
						    if (oldCell!=null)
						    {
						  		newCell= Sigma.U.nextElement(oldCell.parentNode);
								while (grid.isGroupRow(null,newCell)){
									newCell= Sigma.U.nextElement(newCell);
								}
							}
							else
				...
				if (document.getElementById("TipoGrilla").value=="EDITABLE")
				{
					if (newCell && (kcode!=Sigma.Const.Key.ASTE && kcode!=Sigma.Const.Key.NUMLOCK && kcode!=Sigma.Const.Key.F3 && kcode!=Sigma.Const.Key.F1)) 
					{		
						Sigma.U.stopEvent(event);
						var or=oldCell.parentNode,nr=newCell.parentNode;
						if (or!= nr){
							grid._onRowSelect(nr,event);
							Sigma.Grid.handleOverRowCore(event,this,nr);
						}
						var colObj= this.getColumn(this.activeCell);
						if (colObj.id=='operacion')//columna select
						{	
							grid.syncActiveObj(newCell);
							var objMen=document.getElementById("TxtMensajeError");	 
							objMen.focus();  
							grid.startEdit();															
						}
						grid.initActiveObj(event,newCell);		
					}
				}		
 },

NroCuentasCliente : function (sRut, sAbo, sSubProd) {
			try {
				sRut = sRut.toUpperCase();
				sRut = sRut.replace(/\.|-/ig,'');				
...
NroCuentasClientePreAbo : function (sRut, sSubProd) {
			try {
				sRut = sRut.toUpperCase();
				sRut = sRut.replace(/\.|-/ig,'');				
				datastring= "RUT=" + sRut + "&SUBPROD=" + sSubProd;	
				//alert(datastring);	
				jQuery.ajax({ 								
						timeout: 60000,
						dataType: 'JSON',					
			            url: "../jsp/AJAX_NroCuentasClientePreAbo.jsp",
			            data: datastring,	
			            async:false,		          
			            beforeSend:function(xhr){}, 
			            success:function(data){	   			            	
...
SeleccionTipoCargo : function (sTipoJuicio) {
			try {
				datastring= "JUICIO=" + sTipoJuicio;
				//alert(datastring);	
				jQuery.ajax({ 								
						timeout: 60000,
						dataType: 'JSON',					
			            url: "../jsp/AJAX_SeleccionTipoCargo.jsp",
			            data: datastring,	
			            async:false,		          
			            beforeSend:function(xhr){}, 
			            success:function(data){	   			            	
...
ValidaTipoCargo : function (sTipoProducto,sTipoCosta,sJuicio) {
			try {
				...
				jQuery.ajax({ 								
						timeout: 60000,
						dataType: 'JSON',					
			            url: "../jsp/AJAX_ValidaTipoCargo.jsp",
			            data: datastring,	
			            async:false,		          
			            success:function(data){	   			            	
			              try {    
			            	    ...
			            	    //alert("obj.total=" + obj.total);
			              		if (obj.total=='-2' || obj.total=='-10' || obj.total=='-11') {  
				            	   	if (obj.total=='-2' || obj.total=='-10')
				            	   		validResult=[].concat("Tipo de Cargo no v�lido");   
				            	   	else   
				            	   		if (obj.total=='-11')      
...
ValidaTipoCargo : function (sTipoProducto,sTipoCosta,sJuicio, sNroOperacion) {
			try {
				datastring= "TIPOPROD=" + sTipoProducto + "&TIPOCOSTA=" + sTipoCosta + "&JUICIO=" + sJuicio;
				//alert(datastring);
				jQuery.ajax({ 								
...
ValidaNumCheque : function (sCheque) {
			try {
				datastring= "NUMCHEQUE=" + sCheque;
				//alert(datastring);	
				jQuery.ajax({ 								
						timeout: 60000,
						dataType: 'JSON',					
			            url: "../jsp/AJAX_ValidaNumCheque.jsp",
			            data: datastring,	
			            async:false,		          
			            beforeSend:function(xhr){}, 
			            success:function(data){	   			            	
...
ObtenerJuicio : function (sRut) {
			try {
				...
				jQuery.ajax({ 								
						timeout: 60000,
						dataType: 'JSON',					
			            url: "../jsp/AJAX_NroTipoJuicio.jsp",
			            data: datastring,	
			            async:false,		          
			            beforeSend:function(xhr){}, 
			            success:function(data){	             	
			              try {    
			            	    ...
			            	    if (obj.total.length<=0) 
			            	    {			            	    	
			            	    	validResult=[].concat("Rut ingresado no tiene Juicio asignado.");  
			            	    	objMen.value=validResult.join('\n');   
			            	    	objRESPBD.value="";	
			            	    	return false;
			            	    } 
...
startEdit : function(){
			...
			if (!this.readOnly && this.activeCell && this.activeEditor && (this.activeColumn.editable || this.isInsertRow(this.activeRow) )
				...
				this.editing=true;
				//alert("startEdit=" + this.activeValue);
	...
	showEditor : function(value,record){
			...
			//alert("ANTES DE activeEditor.setValue=" + value);
			this.activeEditor.setValue(value,record);
			//alert("DESPUES DE activeEditor.setValue=" + value);
			...
			if (this.activeColumn.id=="operacion")
				document.getElementById("NOMBREOBJETO").value="";
			else	
				document.getElementById("NOMBREOBJETO").value= this.activeColumn.id;
			
	},
	
  validValue : function(colObj,value,record,cell,flag){    	
		if (colObj.editor) {
			...
			{	
				  ...
				  {	
					  ...
					  if (obj!=null)
					  {
					  	   if (obj.value!=objMenError.value)
						   		obj.value="";					  
					  }	
				   }	     
			}
	...
	hideEditor : function(flagmsg){
			var flag=true;
			if (this.editing){
				var row=this.activeRow;
				var oldValue=this.activeValue;
				var value= this.activeEditor.parseValue( this.activeEditor.getValue() );		
				//alert("antes de validValue");				
				var validResult= this.validValue(this.activeColumn,value,this.activeRecord,this.activeCell,flagmsg);	
				//alert("despues de validValue");					
				if (validResult!==true)
						flag=false;
				
				//alert("this.activeValue=" + this.activeValue);
				//alert("value=" + value);
				if (validResult===true &&  String(this.activeValue)!==String(value) ) {
					//alert("despues de validResult==true");		
					var colObj= this.getColumn(this.activeCell);
					//colocar el que ingreso		
					//sino no es select colocar el dato con mascara
					//alert("hideEditor->colObj.id=" + colObj.id);
					if (colObj.id!="operacion")
					{
						var record=this.activeRecord;
						value=record[colObj.fieldIndex];						
					}
	...
	showValidResult : function(value,validResult,cell,colObj){
	   ...
	   validResult=[].concat(validResult);
	 	 //alert("entro aca=" + validResult.join('\n')); 	
	...
	initActiveObj : function(event , newActiveCell){
			...
			//alert("initActiveObj->newActiveCell=" + newActiveCell);	
	...
	initActiveObj_startEditOpe : function(event,newCell,dbl){			
			if ( this.rowNum<1  ){			
				return;
			}
			var p_activeCell=  this.activeCell;					
...
//var cellF =  Sigma.U.getParentByTagName("td",null, event);
	...
	_onBodyClick : function(event,dbl,bodyEl,ets){			
			...
			/*if (ValidaCabecera(document.FRMINGRESOCOS,true))
			{
				this.endEdit(0);
				this.activeMe();
				Sigma.U.stopEvent(event);
		...
		//{
			...
			{
				this.initActiveObj_startEditOpe(event,cell,dbl);	
				...
				if (document.getElementById("TipoGrilla").value=="EDITABLE")
				{
					...
					var colObj= this.getColumn(this.activeCell);	
					var TipoProd=document.getElementById("TIPOPROD");			
					if ((colObj.id=='operacion') && (TipoProd.value=="1" || TipoProd.value=="3" || TipoProd.value=="5"))//solo para tarjeta
					{
						//VOLVER A CARGAR DATOS de operacion
						var recordN=this.getRecordByRow(this.activeRow);
						if (TipoProd.value=="1" || TipoProd.value=="3")
							this.NroCuentasCliente(recordN[1],document.getElementById("ABOGADO").value,document.getElementById("HDDGRUPO").value);
						else
							this.NroCuentasClientePreAbo(recordN[1],document.getElementById("HDDGRUPO").value);
	...
	updateRecordField :function(td,newValue){
		...
		if (colObj ) {
			...
			//alert("record=" + record);
			//alert("colObj.fieldName=" + colObj.fieldName);
...
//load, changePage
	...
	updateFreezeState : function(){
		if (this.frozenColumnList){			
			var i,colObj;
			//alert(this.frozenColumnList.length);
	...
	updateEditState : function(){
		...
		//se comento porque no acepta propiedad o metodo
	...
	deleteRowActive: function(row) {
		if ($('#TipoGrilla').val()=="EDITABLE") {
			//alert('ingreso');
		 	var rows=[].concat(row ||  this.selectedRows );
		 	var record = this.getRecordByRow(row);
		 	var oldCell=mygrid.activeCell;
		 	for (var i=0;i<rows.length ;i=i+1){
				...
				if ( Sigma.$invoke(this,'beforeDelete',[record,row,this])!==false) {			
					var isDel=this.dataset.isDeletedRecord(record);
					 mygrid._nextEditableCell(mygrid.activeCell);
					if (!isDel){
						this.dataset.deleteRecord(record);					
	...
	endEdit : function(flagmsg){
		...
		if (this.activeCell && this.activeEditor && (this.activeColumn.editable || this.isInsertRow(this.activeRow ) ) ) {	
			...
			//alert("antes de hideEditor");
			flag=this.hideEditor(flagmsg);		
			//alert("despues de hideEditor");	
			this.editing=false;
	...
	refresh : function(data){
		...
		function _refresh(){
			if ( Sigma.$invoke(grid,'beforeRefresh',[grid])!==false){
				...
				//selecciono el primer elemento en el refresh		
			    var NumFila=document.getElementById("NUMFILA");
			    var NumReg=document.getElementById("NUMREG");
			    //alert(NumFila.value);
			    //alert(NumReg.value);
				...
				{
					document.getElementById("TxtMensajeError").value="";
					grid.SeleccionaCampoGrilla(NumFila.value,NumReg.value);
					/*
					Sigma.U.stopEvent(event);
				  	grid.unselectAllRow();
				  	//grid.activeMe();				  
				  	var rows=grid.getAllRows();
		 	 		var Cell=rows[NumFila.value].cells[NumReg.value];	
		 	 		//alert("Cell=" + Cell);			
					grid.selectRow(rows[NumFila.value],true);
					grid.activeMe();
					grid.editCell(Cell);	
					*/		
				}		
	...
	save : function(onNav){
		...
		/*alert("url=" + url);
		alert("_insert=" + _insert);
		alert("_update=" + _update);
		alert("_delete=" + _delete);
		*/
...
//		
		...
		Sigma.U.addEvent( d , 'click' ,function(event){			
			Sigma.activeGrid && ( Sigma.activeGrid.endEdit(0) || Sigma.activeGrid.closeGridMenu() );
		});
		Sigma.U.addEvent( d, 'keydown' ,function(event){
			if(Sigma.activeGrid){
				if(Sigma.$invoke(Sigma.activeGrid,'onKeyDown',[event])!==false)
				{
					if(	event.keyCode==144 && (document.getElementById("TIPOPROD").value=="3" || document.getElementById("TIPOPROD").value=="4"))
					{
						if (document.getElementById("Seleccion").value=="ACTIVA")	
