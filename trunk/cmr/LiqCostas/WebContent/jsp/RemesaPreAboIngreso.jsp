...
<%@page import='Seguridad.SessionUsuario'%>
...
<jsp:include page="principal.jsp" flush="false"/>	
  ...
  <SCRIPT LANGUAGE="JavaScript">		
		...
		<% if (resAbogado.size()==0)
		   {%>
		   		arrAbogado[0]={codigo: "PA00801", sucursal: "", nombre: "Pre Abogado 801"};
		   <%}
		   else
		   {	
			for(int i=0; i<resAbogado.size(); i++) {
					resAux = new ArrayList();
	             	...
	             	//alert("<%=resAux.get(1).toString().trim()%>");
	             	//alert("<%=resAux.get(2).toString().trim()%>");
		...
		/*function validarCodAbogado(confoco)
		{
			...
			{	
				...
				if (iPos<0) {	
					...
					f.TxtNomAbo.value=arrAbogado[iPos].nombre;
		...
		function validarNombreAbogado(confoco)
		{
		 	...
		 	if (oAbo.value!="")
			{
				...
				if (iPos<0) {					
					f.NOMBREOBJETO.value="TxtNomAbo";
				    f.TxtMensajeError.value = "Nombre Agencia no se encuentra definido.";
				    f.TxtNomAbo.value="";
					f.HDDNOMABO.value="";
					f.HDDCODABO.value="";
					f.HDDCODABOSUC.value="";
			...
			else
			{
				if (f.ABOGADO.value=="")
				{
					...
					//f.ABOGADO.focus();			
					f.NOMBREOBJETO.value="TxtNomAbo";
					f.TxtMensajeError.value = "Debe ingresar Nombre Agencia.";
					f.TxtNomAbo.select();	
					f.TxtNomAbo.focus();			
					return false;
				}				
  		...
  		function printPageCupon(ifr) 
 		{
  		...
  		function printPageCuponPDF(ifr) {
  			if (ifr.location.href != "about:blank") {
	   		 	...
	   		 	var numRemesa = ifr.document.getElementById("numeroRemesa").value;
	   		 	var fechaRemesa = ifr.document.getElementById("fechaRemesa").value;
		...
		<%if (resDetRemesa.size()>0)
		 {
			for(int i=0; i<resDetRemesa.size(); i++) {
        		...
        		{%>             
    	        arrPaso = new Array("<%=resAux.get(5).toString().trim()%>",valJS.MascaraRut("<%=resAux.get(0).toString().trim()%>"),"<%=resAux.get(1).toString().trim()%>","<%=df.format(Double.parseDouble(resAux.get(2).toString().trim()))%>","<%=df.format(Double.parseDouble(resAux.get(3).toString().trim()))%>","<%=df.format(Double.parseDouble(resAux.get(4).toString().trim()))%>");
			...
			fields :[
				...
				{name : 'Total' , 
					initValue : function(record){
						...
						var avg =(parseFloat(avg2) + parseFloat(avg3));	
			     ...
			     {id: 'rut' , header: "Rut Cliente" , width :150, sortable:false, editable:true, editor:{type:"text",
			     	...
			     	{					     		
							...
							else
							{
									...
									mygrid.NroCuentasClientePreAbo(value,'<%=Grupo%>');
									if (obj.value==document.getElementById("ErrorMen").value)
									{
										//record[1]="";
										f.NOMBREOBJETO.value="";
										flag=false;	
			     		...
			     		validator : function(value,record,colObj,grid)
			     		{
			     			...
			     			{ 
				     		  ...
				     		  else
				     		  {
				     		  	...
				     		  	record[5] = parseInt(valJS.quitaMask(record[3] + '')) + parseInt(valJS.quitaMask(record[4] + ''));
		  ...
		  $(document).ready(function () {
					...
					function findValueCallback(event, data, formatted) {
						...
						if (event.target.id=="ABOGADO") {
								...
								$("#TxtNomAbo").attr("value", data.nombre);	
					...
					$("#ABOGADO").autocomplete(arrAbogado, {
								...
								formatItem: function(row, i, max) {
									...
									return Math.abs(row.codigo) + " - " + row.nombre;
								},
...
<INPUT TYPE="HIDDEN" id="NUMREGTOTAL"  NAME="NUMREGTOTAL" VALUE="5">
<INPUT TYPE="HIDDEN" id="CampoEnter"  NAME="CampoEnter" VALUE="3">
<INPUT TYPE="HIDDEN" id="ErrorMen"  NAME="ErrorMen" VALUE="No existen cuentas asignadas al Rut de Cliente Ingresado.">
  ...
  <tr>
   <td valign="top">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
       	<tr><td align="CENTER" class="titulo"><%=Titulo%> REMESAS ECE PRE-ABOGADO</td></tr>
        <tr>
          <td align="center">
            <div class="tablainterior2">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tablagris">
                 <tr>
		            <td class="labelUno" width="8%">User</td>
				...
				<tr>
		            <!--<td class="labelUno" width="13%" >Código Agencia</td>
		            <td class="labelDos" width="10%">-->
		            <input type="HIDDEN" name="ABOGADO" id="ABOGADO" tabindex="5" maxlength="10" style="width: 100%;margin:auto;" value="<%=CodAbogado%>" onblur="return SetearObjeto('');" onfocus="return SetearObjeto('ABOGADO');">
		            <!--  </td>-->	            
		            <td class="labelUno" width="8%">Agencia</td>
		            ...
		            {%>
		            	<input id="TxtNomAbo" name="TxtNomAbo" style="width: 100%" maxlength="60" type="text" value="<%=NomAbogado%>" onblur="return SetearObjeto('');" onfocus="return SetearObjeto('TxtNomAbo');"/></td>
		            <%}%>
		            </td>
		             <td class="labelDos" width="21%" colspan="2" ></td>
				...
				<tr>
					<td class="labelUno" width="8%">Moneda</td>
		            <td class="labelDos" width="10%">
...
<SCRIPT LANGUAGE="JavaScript">		
	...
	<%}
	  ...
	  {%>
	  	InhabilitarCampos("disabled");
	<%}%>
	//f.ABOGADO.value=arrAbogado[0].codigo;
	//f.TxtNomAbo.value=arrAbogado[0].nombre;
	//f.HDDNOMABO.value=arrAbogado[0].nombre;
	//f.HDDCODABO.value=arrAbogado[0].codigo;
	//f.HDDCODABOSUC.value=arrAbogado[0].sucursal;	
</SCRIPT>	
</BODY>
</HTML>