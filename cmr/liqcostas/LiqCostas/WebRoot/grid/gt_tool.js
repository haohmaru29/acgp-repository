	...
	tools : {
		...
		'del' : { onclick : function(event,grid){ 
		        ...
		        */ 
		        ...
		        //grid.reload();				        
		        //grid.endEdit();     
		        //alert("llego aca del 1");
		       
		        var rows=grid.getAllRows();
						var cell=rows[0].cells[1];
						//grid.editCell(cell);
						grid.selectRow(rows[0],true);
						//Sigma.Grid.handleOverRowCore(event,grid,cell.parentNode);	
						//grid.initActiveObj_startEdit(event,cell);		
						//grid.endEdit();
						//grid.activeMe();				
						grid.editCell(cell);	
						//grid._onRowSelect(cell,event);			
						//grid.activeCell.focus();		
		    		//alert("llego aca del 2");
		    	  return;
			}	
		},
