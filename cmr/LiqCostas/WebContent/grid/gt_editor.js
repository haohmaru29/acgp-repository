		...
		for (var i=0;i<validRule.length;i++ ){
			...
			if (Sigma.$type(rule,'array')&&rule.length>0)	{
				...
				if (validResult!==true){
					...
					//var v_msg=Sigma.Validator.getMessage(this.validRule[i]) ;
					//alert("entro aca 1---" + v_msg);
					errMsg.push(v_msg);
				}
		}
...
Sigma.$extend(Sigma.Editor,{ 
	...
	EDITORS : {
		...
		text : function(editor){
			...
			editor.valueDom=Sigma.$e('input maxlength=20 ',{type:'text',value:editor.defaultValue||'',className:'gt-editor-text'});
		...
		select : function(editor){			
			editor=new Sigma.Editor(editor);
			editor.valueDom=Sigma.U.createSelect( editor.options ,null,{ className:'gt-editor-select' });
			editor.dom.appendChild(editor.valueDom);
			editor.getDisplayValue = function(value){				
				value= value===undefined?this.getValue():value;		
				//alert("getDisplayValue=" + value);		
